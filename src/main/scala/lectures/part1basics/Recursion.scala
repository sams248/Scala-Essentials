...
object Recursion extends App {
  ...
  // With this implementation factorial(5000) will lead to StackOverFlowError
  ...
  def anotherFactorial(n: Int): BigInt = {
    ...
    def factorialHelper(m: Int, accumulator: BigInt): BigInt = {
      if (m <= 1) accumulator else
        factorialHelper(m - 1, m * accumulator) // tail recursion (use recursive call as the last expression)
    }
   ...
   */
  @tailrec
  def concatRec(s: String, n: Int, accumulator: String): String = {
    if (n <= 0) accumulator else
      concatRec(s, n - 1, s + accumulator)
  }

  println(concatRec("Hello", 3, ""))
 ...
 */
  ...
  def isPrime(n: Int): Boolean = {
    ...
    def isPrimeTailRec(m: Int, isStillPrime: Boolean): Boolean = {
      ...
      else if (m <= 1) true else
        isPrimeTailRec(m - 1, n % m != 0 && isStillPrime)
    }
