...
case class ConsV2[+A](h: A, t: MyListV2[A]) extends MyListV2[A] {
  ...
  /*
    [1, 2, 3].map(n * 2)
      = ConsV2(2, [2, 3].map(n * 2))
      = ConsV2(2, ConsV2(4, [3].map(n * 2)))
      = ConsV2(2, ConsV2(4, ConV2(6, EmptyV2.map((n * 2))))
      = ConsV2(2, ConsV2(4, ConV2(6, EmptyV2)))
   */
  def map[B](transformer: MyTransformer[A, B]): MyListV2[B] = {
  ...
  /*
    [1, 2, 3].map(n * 2)
      = ConsV2(2, [2, 3].map(n * 2)
      = ConsV2(2, ConsV2(4, [3].map(n * 2)))
      = ConsV2(2, ConsV2(4, ConsV2(6, EmptyV2.map(n * 2))))
      = ConsV2(2, ConsV2(4, ConsV2(6, EmptyV2)))
   */
  def filter(predicate: MyPredicate[A]): MyListV2[A] = {
